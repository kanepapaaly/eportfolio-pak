...
/** One step of a project still being built. */
...
export type Project = {
  ...
  /** Build steps, for projects in progress. Shown in place of usage figures. */
  roadmap?: RoadmapStep[];
  links: ProjectLink[];
  images: ProjectImage[];
...
export const projects: Project[] = [
  ...
  },
  {
    ...
    name: "Codelight",
    kind: {
      fr: "IA · RAG sur du code",
      en: "AI · RAG over code",
    },
    period: { fr: "Depuis septembre 2026", en: "Since Sep 2026" },
    tagline: {
      fr: "Poser des questions à mes propres dépôts : « j'ai déjà géré ce pattern de concurrence, mais où ? »",
      en: "Asking my own repositories questions: \"I've handled this concurrency pattern before, but where?\"",
    },
    problem: {
      fr: "Retrouver comment j'ai résolu un problème il y a six mois, c'est fouiller mes dépôts avec grep, qui trouve des mots mais pas des intentions. Et un RAG classique découpe le texte par taille fixe : sur du code, il coupe les fonctions en deux et le sens se perd. Codelight traite le code comme du code.",
      en: "Finding how I solved a problem six months ago means digging through my repositories with grep, which finds words, not intent. And a standard RAG splits text into fixed-size chunks: on code, that cuts functions in half and the meaning is lost. Codelight treats code as code.",
    },
    role: {
      fr: "Conception d'un pipeline Python qui tourne entièrement en local. Découpage selon l'arbre syntaxique (tree-sitter) : une fonction ou une classe par morceau, avec sa signature, ses imports et sa docstring. Recherche hybride qui fusionne BM25 et similarité vectorielle, parce qu'un nom exact comme BlockingQueue se trouve mieux par mot-clé. Réponses générées par Qwen2.5-Coder via Ollama, avec le fichier et la fonction cités. Stockage interchangeable : ChromaDB en local, Weaviate pour sa recherche hybride native, afin de comparer ma fusion maison à la sienne.",
      en: "Designing a Python pipeline that runs fully locally. Chunking along the syntax tree (tree-sitter): one function or class per chunk, with its signature, imports and docstring. Hybrid retrieval fusing BM25 and vector similarity, because an exact name like BlockingQueue is better found by keyword. Answers generated by Qwen2.5-Coder through Ollama, citing the source file and function. Swappable storage: ChromaDB locally, Weaviate for its native hybrid search, to benchmark my own fusion against it.",
    },
    result: {
      fr: "Une première version tourne de bout en bout sur mes propres dépôts : je pose une question en langage naturel, Codelight retrouve les fonctions concernées avec leur fichier et rédige une réponse qui les cite. Je suis maintenant dans la phase de peaufinage : mesure de la recherche sur un jeu de questions de référence, réglage de la fusion hybride, qualité des réponses.",
      en: "A first version runs end to end on my own repositories: I ask a question in plain language, Codelight finds the relevant functions with their file and writes an answer that cites them. I'm now in the refinement phase: measuring retrieval on a reference question set, tuning the hybrid fusion, improving answer quality.",
    },
    proves: {
      fr: "Que je sais construire un RAG au-delà du tutoriel : découpage, recherche hybride et évaluation, codés à la main ou appuyés sur des outils établis comme Weaviate, avec une stack open source qui tourne en local.",
      en: "That I can build a RAG beyond the tutorial: chunking, hybrid search and evaluation, hand-built or on established tools like Weaviate, with an open-source stack that runs locally.",
    },
    stack: ["Python", "tree-sitter", "Ollama", "Qwen2.5-Coder", "bge-m3", "BM25", "ChromaDB", "Weaviate", "MCP"],
    roadmap: [
      {
        label: { fr: "Architecture et choix du stack", en: "Architecture and stack choices" },
        state: "done",
      },
      {
        label: { fr: "Chaîne complète sur un dépôt, découpage naïf", en: "End-to-end chain on one repository, naive chunking" },
        state: "done",
      },
      {
        label: { fr: "Découpage par AST avec tree-sitter", en: "AST chunking with tree-sitter" },
        state: "done",
      },
      {
        label: { fr: "Recherche hybride BM25 + vecteurs, maison puis Weaviate", en: "Hybrid BM25 + vector search, hand-built then Weaviate" },
        state: "done",
      },
      {
      ...
      {
        label: { fr: "Serveur MCP pour l'interroger depuis le terminal", en: "MCP server to query it from the terminal" },
        state: "done",
      },
      {
        label: { fr: "Peaufinage : pertinence des résultats et réglage de la recherche hybride", en: "Refinement: result relevance and hybrid search tuning" },
        state: "current",
      },
    ],
    statusTag: { fr: "En peaufinage", en: "In refinement" },
    links: [],
    images: [],
  },
  {
